...
export async function POST(request) {
  try {
    ...
    const { assessmentId, studentName, studentData, answers } = body;
    ...
    // ── Fetch questions ────────────────────────────────────────────────────
    // Columns that actually exist: id, type, question_type, text, options, answer, answer_template
    const { data: questions, error: questionsError } = await supabase
      .from('questions')
      .select('id, type, question_type, answer, options, answer_template')
    ...
    const { data: fullQuestions } = await supabase
      .from('questions')
      .select('id, type, question_type, text, options, answer, explanation, hint, answer_template, order_index')
      .eq('assessment_id', assessmentId)
      .order('order_index');
