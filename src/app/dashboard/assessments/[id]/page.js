            ...
            return (
              ...
              >
                <div className={cn(
                  ...
                  isCorrect ? 'bg-success-light/40' : 'bg-danger-light/40'
                )}>
                  <div className={cn(
                      ...
                      {q.options.map((opt, oi) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {isAnswer && (
                              <span className="text-xs font-bold ml-auto">✓ Correct</span>
                            )}
                            {!isAnswer && isStudent && (
                              <span className="text-xs font-bold ml-auto">Student&apos;s answer</span>
                            )}
                          </div>
                        )
                      })}
...
export default function AssessmentDetailPage({ params }) {
  ...
  const handleSaveTitle = async () => {
    ...
    if (error) {
      toast({ message: 'Failed to update title.', type: 'error' })
    } else {
      toast({ message: 'Title updated!', type: 'success' })
      ...
      <div className="flex flex-col gap-6 max-w-4xl mx-auto">
        ...
        <div className="bg-white border border-border rounded-2xl p-6 shadow-card">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div className="flex-1 min-w-0">
              ...
              {editingTitle ? (
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => { setEditingTitle(false); setTitleValue(assessment.title) }}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="text-ink-4 hover:text-brand-600 transition-colors"
                    title="Edit title"
                  >
                    <Pencil size={15} />
                  </button>
            ...
            {/* Action buttons — clean, no raw URL */}
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button
                onClick={copyLink}
                className="inline-flex items-center gap-2 bg-white border border-border text-ink-2 text-sm font-semibold px-4 py-2.5 rounded-xl hover:bg-surface transition-colors"
              >
          ...
          {assessment.submissions.length === 0 ? (
            <div className="bg-white border border-dashed border-border rounded-2xl p-8 text-center">
              ...
              <p className="text-sm text-ink-3">
                Share the link with students to get started
              </p>
            </div>
          ) : (
            <div className="bg-white border border-border rounded-2xl overflow-hidden shadow-card">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-surface border-b border-border">
                    ...
                    <th className="px-5 py-3 text-xs font-semibold uppercase tracking-widest text-ink-4 text-center">
                      Review
                    </th>
                  </tr>
                </thead>
                        ...
                        <div className="flex items-center gap-2">
                          <div className="w-14 h-1.5 bg-border rounded-full overflow-hidden hidden sm:block">
                            <div
                              className={cn(
                                'h-full rounded-full',
                                (sub.score ?? 0) >= 75 ? 'bg-success' :
                                (sub.score ?? 0) >= 50 ? 'bg-amber'   : 'bg-danger'
                              )}
